...
| `CHANNEL_OPEN_REQUEST framing` | Sent on the **target service's own channel**, not channel 0, with flags `0x0F` = `BULK\|ENCRYPTED\|CONTROL`. On channel 0 the CONTROL bit stays clear (`0x0B`). | **The bug that stopped a real car dead, and the capture that settles the CONTROL-bit disagreement recorded in `FrameHeader.kt`.** Headway sent `0x0B` on the service channel; a 2021 Chevrolet Infotainment 3 unit closed both the TCP and RFCOMM links ~30 ms later, with no TLS alert and no error frame, 11 times out of 11, immediately after sending its service list. Four references agree on the correct shape: aasdk handles `CHANNEL_OPEN_REQUEST` only inside per-service channels and never in `ControlServiceChannel`; AACS's phone role sends `sendToHeadunit(channelId, Bulk\|Encrypted\|Specific, ...)` (its `Specific` is the bit aasdk calls `CONTROL`); aa-proxy-rs derives `let control_flag = if channel == 0 { 0 } else { CONTROL_FLAG };` from observed traffic and notes "Without CONTROL (0x04), Android may not treat our synthetic CHANNEL_OPEN_RESPONSE as a channel-control frame". | `aasdk/src/Channel/Sensor/SensorServiceChannel.cpp` L74 (+7 more); `AACS/AAServer/src/ChannelHandler.cpp` L34-L46; `aa-proxy-rs/src/bt_real_hu_passthrough.rs` L131, `src/mitm.rs` L3999-L4004; real-vehicle capture 2026-08-13 16:14:44 |
| `PING_RESPONSE encryption` | Encrypted whenever the TLS session is up, regardless of how the request arrived | Headway previously mirrored the request's flag, which no reference does. AACS's phone role — the only phone-side implementation known to work against real head units — always encrypts. aasdk sends *plain* pings, but aasdk is the head unit, so that is evidence about what a phone must tolerate, not what a car accepts. | `AACS/AAServer/src/AaCommunicator.cpp` L252-L268; contrast `aasdk/src/Channel/Control/ControlServiceChannel.cpp` L117-L133 |
| `service discovery polarity (confirmed one-way)` | Phone sends `ServiceDiscoveryRequest`; head unit answers `ServiceDiscoveryResponse`. The phone never sends a response. | Checked because Gearhead logs `Adding service (Service id=6 type=BluetoothService {carAddress=...})`, which reads like the phone publishing a list. It is not — it is the phone *ingesting* the car's list and instantiating a handler per service, exactly as AACS does. `BluetoothService.car_address` is a field only the car can populate. aasdk's `IControlServiceChannel` has `sendServiceDiscoveryResponse` and no request sender; its event handler has `onServiceDiscoveryRequest` and no response handler. | `aasdk/include/aasdk/Channel/Control/IControlServiceChannel.hpp` L52-L61; `AACS/AAServer/src/AaCommunicator.cpp` L106-L128, L237-L242; `aa-proxy-rs/src/mitm.rs` L2947-L2950 |
...
| `keepalive latency budget` | The head unit states its own terms in `ServiceDiscoveryResponse.connection_configuration.ping_configuration`: openauto fills `{tracked_ping_count=5, timeout_ms=3000, interval_ms=1000, high_latency_threshold_ms=200}`. A phone that answers later than `timeout_ms` is dropped, and the protocol has `STATUS_PING_TIMEOUT = -25` for it. | **The reason a send path may not be one FIFO lock.** `Mutex` in kotlinx.coroutines is fair, so a `PING_RESPONSE` queues behind whatever media is already waiting. That is a handful of senders rather than one per message — video, each audio sink, sensors and media status each drive the wire from a single coroutine, and video discards frames rather than queueing them — but the lock is held for a *whole* message, and a keyframe is several 16 KiB fragments on a radio the same log shows dropping 810 video frames in eight minutes. Note this does not bound the sharper case: one `transport.write` that blocks for a long time holds the lock whatever lane the waiter is in, and Java's `SO_TIMEOUT` bounds reads and not writes. The 2026-08-19 drive log against the target vehicle ends three sessions with `Connection reset`, `Broken pipe` and `Connection reset`, none of them preceded by `BYEBYE_REQUEST`, which is what a unit that gave up waiting looks like from the phone; the same log shows 810 video frames dropped in eight minutes for want of wire. `FramedConnection` therefore gives channel 0 its own lane past the bulk queue, and the session summary reports the slowest answer against the car's announced budget so the next log settles it either way. Headway had parsed `ping_configuration` since the schema was generated and never read it. | `docs/protocol-notes.md` STEP 6 (openauto `AndroidAutoEntity.cpp` L172-L213); `aap_protobuf/service/control/message/PingConfiguration.proto`; `Status.proto` `STATUS_PING_TIMEOUT = -25`; real-vehicle drive log 2026-08-19 08:45:12, 08:50:39, 08:51:20 |
